# -*- coding: utf-8 -*-
"""
Analysis for the TofuEval-MeetingBank second benchmark. Mirrors the RAGTruth
analysis: for each scorer, grounding (GASP) detection AUC with bootstrap 95% CIs
at the response and span levels (span grouped by summary), against perplexity and
length, plus the training-free threshold-only variant and significance vs
perplexity.
"""
...
BASE = str(RESULTS)
TAGS = ["tofueval_Qwen2.5-0.5B-Instruct", "tofueval_Qwen2.5-1.5B-Instruct", "tofueval_SmolLM2-1.7B-Instruct"]

def threshold_only(d, grouped):
    y = d["label"].to_numpy(); oof = np.zeros(len(d))
    if grouped:
        cv = StratifiedGroupKFold(5, shuffle=True, random_state=42).split(d[GROUND], y, d["resp_id"])
    else:
        cv = StratifiedKFold(5, shuffle=True, random_state=42).split(d[GROUND], y)
    for tr, te in cv:
        mu = d[GROUND].iloc[tr].mean(); sd = d[GROUND].iloc[tr].std() + 1e-9
        oof[te] = ((d[GROUND].iloc[te] - mu) / sd).sum(axis=1).to_numpy()
    g = d["resp_id"].to_numpy() if grouped else None
    return boot_auc_ci(y, -oof, g)

def main():
    lines = ["# TofuEval-MeetingBank second benchmark (grounding detection)\n"]
    for tag in TAGS:
        ...
        lines.append(f"## {tag}  (summaries={len(rdf)}, sentences={len(sdf)}, halluc={int(sdf.label.sum())})")
        ...
        lines.append("")
    out = os.path.join(BASE, "TOFUEVAL_RESULTS.md")
